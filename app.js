...
var $author$project$Main$GotPhotos = function (a) {
	return {$: 'GotPhotos', a: a};
};
var $author$project$Main$catsApiUrl = 'https://api.thecatapi.com/v1/breeds?limit=10';
var $author$project$Main$CatsData = F2(
	function (photo, breedName) {
		return {breedName: breedName, photo: photo};
	});
...
var $author$project$Main$catsDecoder = A3(
	...
	A3(
		...
		_List_fromArray(
			['image', 'url']),
		$elm$json$Json$Decode$string,
		$elm$json$Json$Decode$succeed($author$project$Main$CatsData)));
...
var $author$project$Main$fetchCats = function (msg) {
	return A3(
		$ohanhi$remotedata_http$RemoteData$Http$get,
		$author$project$Main$catsApiUrl,
		msg,
		$elm$json$Json$Decode$list($author$project$Main$catsDecoder));
};
var $author$project$Main$initialCmd = $author$project$Main$fetchCats($author$project$Main$GotPhotos);
var $author$project$Main$initialModel = {beingDragged: $elm$core$Maybe$Nothing, catsGame: _List_Nil, error: $elm$core$Maybe$Nothing};
...
var $author$project$Main$updateDropped = F3(
	function (draggedItem, droppedOnPhoto, catsGame) {
		var updateBreedName = F3(
			function (breedName, photo, cat) {
				return _Utils_eq(cat.photo, photo) ? _Utils_update(
					cat,
					{breedName: breedName}) : cat;
			});
		var maybeDroppedOnCat = $elm$core$List$head(
			A2(
				$elm$core$List$filter,
				function (cat) {
					return _Utils_eq(cat.photo, droppedOnPhoto);
				},
				catsGame));
		var maybeDraggedCat = $elm$core$List$head(
			A2(
				$elm$core$List$filter,
				function (cat) {
					return _Utils_eq(cat.breedName, draggedItem);
				},
				catsGame));
		var _v0 = _Utils_Tuple2(maybeDraggedCat, maybeDroppedOnCat);
		if ((_v0.a.$ === 'Just') && (_v0.b.$ === 'Just')) {
			var draggedCat = _v0.a.a;
			var droppedOnCat = _v0.b.a;
			return A2(
				$elm$core$List$map,
				A2(updateBreedName, droppedOnCat.breedName, draggedCat.photo),
				A2(
					$elm$core$List$map,
					A2(updateBreedName, draggedItem, droppedOnPhoto),
					catsGame));
		} else {
			return catsGame;
		}
	...
	function (msg, model) {
		switch (msg.$) {
				...
				switch (response.$) {
					...
					case 'Success':
						if (!response.a.b) {
							...
							var catsData = response.a;
							return _Utils_Tuple2(
								model,
								A2(
									$elm$random$Random$generate,
									$author$project$Main$NewGame,
									$elm_community$random_extra$Random$List$shuffle(catsData)));
						}
					default:
			...
			case 'NewGame':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							catsGame: A2($elm$core$List$take, 4, catsData)
						}),
					$elm$core$Platform$Cmd$none);
			case 'SubmitGame':
				return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
			case 'Drag':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							beingDragged: $elm$core$Maybe$Just(item)
						}),
					$elm$core$Platform$Cmd$none);
			case 'DragEnd':
				return _Utils_Tuple2(
			...
			default:
				...
				var _v2 = model.beingDragged;
				if (_v2.$ === 'Nothing') {
					return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
				} else {
					var draggedItem = _v2.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								beingDragged: $elm$core$Maybe$Nothing,
								catsGame: A3($author$project$Main$updateDropped, draggedItem, newPosition, model.catsGame)
							}),
						$elm$core$Platform$Cmd$none);
				}
		}
...
var $author$project$Theme$black = A3($mdgriffith$elm_ui$Element$rgb255, 0, 0, 0);
...
var $author$project$Theme$pink = A3($mdgriffith$elm_ui$Element$rgb255, 150, 50, 150);
var $author$project$Main$SubmitGame = {$: 'SubmitGame'};
...
var $author$project$Main$viewBreed = function (breedName) {
	return $mdgriffith$elm_ui$Element$text(breedName);
};
var $author$project$Main$viewDraggableItem = function (item) {
	return A2(
		...
		_List_fromArray(
			[
				...
				$author$project$Main$Drag(item.breedName)),
				$author$project$Main$onDragEnd($author$project$Main$DragEnd)
			]),
		$author$project$Main$viewBreed(item.breedName));
};
var $author$project$Main$DragOver = {$: 'DragOver'};
var $author$project$Main$Drop = function (a) {
	return {$: 'Drop', a: a};
};
var $author$project$Theme$blue = A3($mdgriffith$elm_ui$Element$rgb255, 51, 102, 255);
...
var $author$project$Main$viewDroppableArea = F2(
	function (_v0, elem) {
		var photo = _v0.photo;
		return A2(
			$mdgriffith$elm_ui$Element$el,
			_List_fromArray(
				[
					...
					$author$project$Main$Drop(photo))
				]),
			elem);
	});
...
var $author$project$Main$viewPhoto = function (url) {
	return A2(
		...
		{description: 'cat\'s photograph', src: url});
};
var $author$project$Main$viewGameRow = function (cat) {
	return A2(
		$mdgriffith$elm_ui$Element$row,
		_List_Nil,
		_List_fromArray(
			[
				$author$project$Main$viewPhoto(cat.photo),
				A2(
				$author$project$Main$viewDroppableArea,
				cat,
				$author$project$Main$viewDraggableItem(cat))
			]));
};
var $author$project$Main$viewCatsGame = function (game) {
	return A2(
		...
		_List_fromArray(
			[
				...
				$mdgriffith$elm_ui$Element$column,
				_List_Nil,
				A2($elm$core$List$map, $author$project$Main$viewGameRow, game)),
				A2(
				$mdgriffith$elm_ui$Element$Input$button,
				_List_Nil,
				{
					label: $mdgriffith$elm_ui$Element$text('Ready!'),
					onPress: $elm$core$Maybe$Just($author$project$Main$SubmitGame)
				})
			]));
};
var $author$project$Error$readHttpError = function (error) {
	switch (error.$) {
		case 'BadUrl':
			var str = error.a;
		...
		case 'BadStatus':
			var code = error.a;
			return 'Bad staus error with code ' + $elm$core$String$fromInt(code);
		default:
			var str = error.a;
...
var $author$project$Theme$smallFont = $mdgriffith$elm_ui$Element$Font$size(12);
var $author$project$Error$viewHttpError = function (maybeError) {
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					$author$project$Theme$smallFont
				]),
			$mdgriffith$elm_ui$Element$text(
				$author$project$Error$readHttpError(error)));
	}
};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				$mdgriffith$elm_ui$Element$Background$color($author$project$Theme$pink),
				$mdgriffith$elm_ui$Element$width(
				A2($mdgriffith$elm_ui$Element$minimum, 600, $mdgriffith$elm_ui$Element$shrink))
			]),
		_List_fromArray(
			[
				$author$project$Error$viewHttpError(model.error),
				$author$project$Main$viewCatsGame(model.catsGame)
			]));
};
...
var $author$project$Theme$defaultFonts = $mdgriffith$elm_ui$Element$Font$family(
	_List_fromArray(
		[
...
var $author$project$Main$header = A2(
	...
	A2(
		...
		$mdgriffith$elm_ui$Element$text('Cats 😻')));
...
var $author$project$Main$body = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				$mdgriffith$elm_ui$Element$width($mdgriffith$elm_ui$Element$fill),
				$mdgriffith$elm_ui$Element$Background$color($author$project$Theme$black),
				$author$project$Theme$defaultFonts
			]),
		A2(
			$mdgriffith$elm_ui$Element$column,
			_List_fromArray(
